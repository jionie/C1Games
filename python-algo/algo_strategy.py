...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def create_my_hierarchical_defenses(self):
        ...
        self.regional_complete_defenses['[0, 1]'][DESTRUCTOR] += \
            [[3, 12], [4, 12], [5, 12]]
        self.regional_complete_defenses['[0, 1]'][ENCRYPTOR] += \
            [[4, 11], [5, 11], [5, 10]]
        ...
        self.regional_complete_defenses['[1, 1]'][DESTRUCTOR] += \
            [[6, 11], [7, 10], [8, 9], [9, 8], [10, 7], [11, 6], [12, 5], [13, 4]]

        # [2, 1]
        self.regional_complete_defenses['[2, 1]'][FILTER] += \
            [[14, 5], [15, 6], [16, 7], [17, 8], [18, 9], [19, 10], [20, 12], [21, 12]]
        self.regional_complete_defenses['[2, 1]'][ENCRYPTOR] += \
            [[14, 2], [14, 3], [15, 3], [15, 4], [16, 4], [16, 5], [17, 5], [17, 6], [18, 6], [18, 7],
                [19, 7], [19, 8], [20, 8], [20, 9], [21, 9], [21, 10], [22, 10]]
        self.regional_complete_defenses['[2, 1]'][DESTRUCTOR] += \
            [[14, 4], [15, 5], [16, 6], [17, 7], [18, 8], [19, 9], [20, 10], [20, 11], [21, 11], [22, 11]]
        ...
        # v0: skeleton
        self.defense_v0[FILTER] = [[1, 13], [2, 13]]

        self.defense_v0[DESTRUCTOR] = [[1, 12], [2, 12], [3, 12], [25, 12], [26, 12], [[22, 11]]]
        
        self.defense_v0[ENCRYPTOR] =  [[4, 11], [5, 10], [22, 10], [6, 9], [21, 9], [7, 8], [20, 8], [8, 7], \
                                        [19, 7], [9, 6], [18, 6], [10, 5], [17, 5], [11, 4], [16, 4], [12, 3], \
                                            [15, 3], [13, 2], [14, 2]]


        # v1
        self.defense_v1[FILTER] = [[0, 13], [20, 12], [21, 12], [23, 13], [24, 13], [25, 13], [26, 13], [27, 13]]
        self.defense_v1[DESTRUCTOR] = [[24, 12], [19, 11], [19, 12], [20, 11], [21, 11], [4, 12], [5, 12]]

        # v2
        self.defense_v2[FILTER] = [[3, 13], [4, 13], [5, 13]]

        # v3
        self.defense_v3[FILTER] = [[19, 10], [18, 9], [17, 8], [16, 7], [15, 6], [14, 5]]
        self.defense_v3[DESTRUCTOR] = [[20, 10], [19, 9], [18, 8], [17, 7], [16, 6], [15, 5], [14, 4]]
        self.defense_v3[ENCRYPTOR] = [[21, 10], [20, 9], [19, 8], [18, 7], [17, 6], [16, 5], [15, 4], [14, 3]]

        # v4
        self.defense_v4[FILTER] = [[13, 5], [12, 6], [11, 7], [10, 8], [9, 9], [8, 10], [7, 11], [6, 12]]
        self.defense_v4[DESTRUCTOR] = [[13, 4], [12, 5], [11, 6], [10, 7], [9, 8], [8, 9], [7, 10], [6, 11]]
        self.defense_v4[ENCRYPTOR] = [[13, 3], [12, 4], [11, 5], [10, 6], [9, 7], [8, 8], [7, 9], [6, 10]]
    ...
    def build_skeleton(self, game_state):
        ...
        # deploy skeleton filters as many as we can
        for filter_location in self.defense_v0[ENCRYPTOR]:
            game_state.attempt_spawn(ENCRYPTOR, filter_location)
    ...
    def attempt_first_attack(self, game_state):
        ...
        EMP_start_locations = [[3, 10]]
        scrambler_start_locations = [[18, 4], [22, 8]]
    ...
    def deploy_defense(self, game_state, damages):
        ...
        for location in self.defense_v0[FILTER]:
            game_state.attempt_spawn(FILTER, location)
        for location in self.defense_v0[ENCRYPTOR]:
            game_state.attempt_spawn(ENCRYPTOR, location)

        # for v1: key filters that lead the way
        for location in self.defense_v1[DESTRUCTOR]:
            game_state.attempt_spawn(DESTRUCTOR, location)
        ...
        # for v3: only filters
        for location in self.defense_v3[DESTRUCTOR]:
            game_state.attempt_spawn(DESTRUCTOR, location)
        for location in self.defense_v3[ENCRYPTOR]:
            game_state.attempt_spawn(ENCRYPTOR, location)
        ...
        if self.my_previous_health - self.my_current_health >= my_health_damage_threshold:
            ...
            if front_regional_keys[ordered_indices[0]] in ['[0, 1]', '[1, 1]']:
                ...
                if self.my_current_resources['BITS'] >= 10:
                    ...
                    game_state.attempt_spawn(SCRAMBLER, [18, 4], num_of_scramblers)
                # else:
            ...
            # deploy attack
            primary_position = [[3, 10]]
            auxiliary_position = [[18, 4], [22, 8]]
            if attack_type:
                # use EMPs + scrambler
                ...
                game_state.attempt_spawn(EMP, primary_position[attack_direction], 2)
                game_state.attempt_spawn(PING, primary_position[attack_direction], num_of_unit)
                game_state.attempt_spawn(SCRAMBLER, auxiliary_position[0], 1)
